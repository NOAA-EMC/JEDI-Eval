...
def bin_df(df, variable, dlat=5, dlon=5, return_count=False):
    """
    Bin a variable from a dataframe given dlat, dlon using xhistogram
    Input:
    ...
    count = histogram(bindata_chunked['latitude'],
                        bindata_chunked['longitude'],
                        bins=[lat_bins, lon_bins])

    # create bins with count
    bindata = histogram(bindata_chunked['latitude'],
                        bindata_chunked['longitude'],
                        bins=[lat_bins, lon_bins],
...
def spatial_binned(binned_data, metadata):
    ...
    cs = plt.pcolormesh(binned_data['longitude_bin'], binned_data['latitude_bin'],
...
def spatial(df, metadata, variable=None):
    ...
    if variable is not None:
        ...
        cs = plt.scatter(df['longitude'], df['latitude'], s=10,
